...
def build_family_handlers(args, consensus_writer, annotated_writer, logger):
...
class _WriteAnnotatedAlignsHandler:
    def __init__(self, writer):
        self._writer = writer

    def handle(self, family):
        for align_pair in sorted(family.align_pairs, key=lambda a: a.query_name):
            self._writer.write(family, align_pair, align_pair.left)
            self._writer.write(family, align_pair, align_pair.right)
...
class _FamilySizeStatHandler:
    ...
    def end(self):
        ...
        if len(self.collection) < 1:
            self._log.warning("Empty Family Collection.  Confirm format of BAM file and Min_Family_Size")
...
class _MatchStatHandler:
    ...
    def end(self):
        ...
        self._log.debug("{:.2f}% ({}/{}) original pairs matched UMT exactly",
                        100 * (1 - self.percent_inexact_match),
                        exact_count,
                        self.total_pair_count)

        self._log.debug(("{:.2f}% ({}/{}) original pairs matched by Hamming distance threshold (<={}) on left or right "
                         "UMT "), 100 * self.percent_inexact_match, self.total_inexact_match_count,
                        self.total_pair_count, self.hamming_threshold)
